...
class dummytlsPanel(wx.Panel):
    ...
    def __init__(self, parent, laser, graphPanel):
        super(dummytlsPanel, self).__init__(parent)
        self.devName = 'Dummy Laser'
        self.laser = laser
...
class dummylaserTopPanel(wx.Panel):
    ...
    def __init__(self, parent, laser, showGraph, ishome):
        super(dummylaserTopPanel, self).__init__(parent)
    ...
    def InitUI(self):
        ...
        if self.ishome:
            ...
            self.detectorPanel = detectorPanel(self, 4, self.laser, self.ishome)
...
class laserPanel(wx.Panel):
    ...
    def InitUI(self):
        ...
        if self.ishome:
            ...
            self.laserPanel = dummytlsPanel(self, self.laser, self.graphPanel)
            self.detectorPanelLst = list()

            # for ii in xrange(self.laser.getNumPWMChannels()):
            self.detectorPanel = detectorPanel(self, 3, self.laser, True)
            self.laserPanel.detectorPanel = self.detectorPanel
...
class detectorPanel(wx.Panel):
    ...
    def InitUI(self):
        ...
        if self.detectflag:
            ...
            self.detectorPanelLst = list()
            for ii, slotInfo in zip(range(3), range(3)):
                name = 'Slot %d Det %d' % (slotInfo, slotInfo + 1)
                det = individualDetPanel(self, name=name, slot=slotInfo, chan=slotInfo)
